...
class MSGAN(object):
    ...
    def __init__(self, model='distgan', \
        ...
        if self.verbose == True:
            print('[msdistgan.py -- __init__] lambda_p = %f' % (self.lambda_p))
    ...
    def create_discriminator(self):
        ...
        elif self.nnet_type == 'dcgan' and self.db_name in ['cifar10','cifar100']:
            return discriminator_dcgan_cifar
        elif self.nnet_type == 'sngan' and self.db_name in ['cifar10','cifar100']:
            return discriminator_sngan_cifar            
        elif self.nnet_type == 'sngan' and self.db_name == 'stl10':
            return discriminator_sngan_stl10
        elif self.nnet_type == 'resnet' and self.db_name in ['cifar10','cifar100']:
    ...
    def create_model(self):
        ...
        # Discriminator loss with hinge loss function
            ...
            if self.nnet_type == 'dcgan':
                ...
                self.g_cost_gran = - tf.reduce_mean(tf.minimum(0.,-1 + self.d_fake_logit)
            else:
                ...
                self.g_cost_gran = - tf.reduce_mean(tf.minimum(0.,-1 + self.d_fake_sigmoid))
            ...
            for step in range(self.n_steps + 1):
                ...
                if step % self.log_interval == 0:
                    if self.verbose:
                       ...
                       if self.ss_task > 0:
                          output_str = '[msdistgan.py -- train] step: {:4d}, D loss: {:8.4f}, D loss (gan): {:8.4f}, D loss (acc): {:8.4f} G loss: {:8.4f}, G loss (gan): {:8.4f}, G loss (acc): {:8.4f}, time: {:3d} s'.format(step, loss_d, loss_d_gan, loss_d_acc, loss_g, loss_g_gan, loss_g_acc, elapsed)
                       else:
